...
func makeTestRow(vs ...interface{}) (row Row) {
	for _, v := range vs {
...
func TestTableExpr(t *testing.T) {
	...
	r, err = db.ExecStmt(parseStmt(t, s))
	require.Nil(t, err)
	require.Equal(t, []Row{makeTestRow(2, "ba")}, r.Values)
	...
	r, err = db.ExecStmt(parseStmt(t, s))
	require.Nil(t, err)
	require.Equal(t, []Row{makeTestRow(-1, 1, "ba", "b")}, r.Values)
}
...
func TestTableIndices(t *testing.T) {
	...
	r, err = db.ExecStmt(parseStmt(t, s))
	require.Nil(t, err)
	require.Equal(t, []Row{makeTestRow(0, 3), makeTestRow(1, 2), makeTestRow(2, 2)}, r.Values)
	...
	r, err = db.ExecStmt(parseStmt(t, s))
	require.Nil(t, err)
	require.Equal(t, []Row{makeTestRow(0, 3), makeTestRow(1, 1), makeTestRow(2, 0)}, r.Values)
	...
	r, err = db.ExecStmt(parseStmt(t, s))
	require.Nil(t, err)
	require.Equal(t, []Row{makeTestRow(2, 0)}, r.Values)
}
