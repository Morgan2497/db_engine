...
func renderExprPrefix(expr interface{}) string {
	...
	case *ExprTuple:
		...
		for i, child := range node.kids {
			parts[i] = renderExprPrefix(child)
		}
...
func TestParseTuple(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:  "column tuple",
			query: "(a, b)",
			expected: &ExprTuple{kids: []interface{}{
				"a",
				"b",
			}},
		},
		...
		{
			name:  "parenthesized expression remains grouped",
			query: "(a + b)",
			expected: &ExprBinOp{
				op:    OP_ADD,
				left:  "a",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			t.Logf("[INPUT] %q", test.query)
			t.Logf("[TREE]\n%s", renderExprTree(expr))
			t.Logf("[PREFIX] %s", renderExprPrefix(expr))
			t.Logf("[CURSOR] position=%d input length=%d", parser.pos, len(parser.buf))

			require.NoError(t, err)
			assert.True(t, parser.isEnd())
			assert.Equal(t, test.expected, expr)
		})
	}
}

func TestParseTupleComparison(t *testing.T) {
	query := "(a, b) > (1, 2)"
...
func TestParseTupleRejectsMalformedInput(t *testing.T) {
	for _, query := range []string{"(a, b", "(a,)", "()"} {
		t.Run(query, func(t *testing.T) {
			parser := NewParser(query)
			expr, err := parser.parseExpr()
			t.Logf("[INPUT] %q", query)
			t.Logf("[PARTIAL RESULT] expression=%v cursor=%d", expr, parser.pos)
			t.Logf("[ERROR] %v", err)
			assert.Error(t, err)
		})
	}
