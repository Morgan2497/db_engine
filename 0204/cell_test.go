...
func TestCellI64Roundtrip(t *testing.T) {
	t.Log("=== 0204 Cell I64 roundtrip test start ===")
	...
	t.Logf("[ENCODE check] expect 8 bytes; two's complement LE for -42 → D6 FF FF FF FF FF FF FF")
	assert.Len(t, buf, 8)

	decoded := Cell{Type: TypeI64}
	t.Log("calling Decode — Type must be TypeI64 before decode (schema supplies this later)")
	...
	t.Log("=== 0204 Cell I64 roundtrip test end ===")
}

func TestCellStrRoundtrip(t *testing.T) {
	t.Log("=== 0204 Cell Str roundtrip test start ===")
	...
	t.Logf("[DECODE note] size=5 came from buf[0:4], not from a global constant")
	assert.NoError(t, err)
	assert.Empty(t, rest)
	assert.Equal(t, []byte("hello"), decoded.Str)

	t.Log("=== 0204 Cell Str roundtrip test end ===")
}

func TestCellChainedDecode(t *testing.T) {
	t.Log("=== 0204 chained cell decode test start ===")
	t.Log("same pattern Row.EncodeVal uses: append cells into one buffer, decode with rest")

	t.Log("--- encode phase: one buffer, two cells ---")
	...
	t.Log("[ENCODE layout] [8-byte i64][4-byte len=1][byte 'x']")

	t.Log("--- decode phase: consume prefix, pass rest to next cell ---")
	...
	t.Log("=== 0204 chained cell decode test end ===")
}

func TestCellEncodeAppend(t *testing.T) {
	t.Log("=== 0204 encode append (toAppend) test start ===")

	base := []byte("prefix")
	t.Logf("[SETUP] toAppend=%q len=%d", base, len(base))
	t.Log("[EXPECT] Encode appends after existing bytes — does not replace prefix")

	buf := (&Cell{Type: TypeI64, I64: 7}).Encode(base)
	t.Logf("[ENCODE output] len=%d bytes=%v", len(buf), buf)
	t.Logf("[ENCODE layout] [prefix 6 bytes][i64(7) 8 bytes]")

	assert.True(t, len(buf) > len(base))
	assert.Equal(t, "prefix", string(buf[:6]))
	t.Logf("[CHECK] buf[:6]=%q preserved; i64 bytes at buf[6:]", string(buf[:6]))

	t.Log("=== 0204 encode append test end ===")
}
