...
```text
Schema:  "Column 1 is string named src, it's part of the PK"
              ↓ maps / guides
Row:     row[1] = Cell{"a"}     ← actual value lives HERE
```

Schema **maps out** the shape — it does **not** contain row data or `Cell` values.

---

### 1b. Visual: Schema maps · Row holds data

#### One schema, many rows

```text
                    ┌─────────────────────────────────────┐
                    │  SCHEMA  (blueprint — defined once)   │
                    │  Table: "link"                       │
                    │                                      │
                    │  idx │ Cols (metadata)  │ PK?        │
                    │  ────┼──────────────────┼────        │
                    │   0  │ time  → i64     │            │
                    │   1  │ src   → str     │  ✓         │
                    │   2  │ dst   → str     │  ✓         │
   ...
   ├───────────────┤           ├───────────────┤           ├───────────────┤
   │[0] Cell: 123  │           │[0] Cell: 456  │           │[0] Cell: 789  │
   │[1] Cell: "a"  │           │[1] Cell: "a"  │           │[1] Cell: "x"  │
   │[2] Cell: "b"  │           │[2] Cell: "c"  │           │[2] Cell: "y"  │
   └───────────────┘           └───────────────┘           └───────────────┘
        actual data                 actual data                 actual data
```

#### Index alignment (same index, different roles)

```text
SCHEMA (what each index MEANS)          ROW (what each index HOLDS)
─────────────────────────────          ─────────────────────────────

idx 0  ──maps to──►  Cols[0]: time/i64  ──►  row[0]: Cell { I64: 123 }
idx 1  ──maps to──►  Cols[1]: src/str   ──►  row[1]: Cell { Str: "a" }
idx 2  ──maps to──►  Cols[2]: dst/str   ──►  row[2]: Cell { Str: "b" }

         │                                        │
         │  name + type only                      │  real values
         │  (`Column` struct)                     │  (`Cell` struct)
         └──────────────── same index ────────────┘
```

#### `Column` vs `Cell` (not the same thing)

```text
┌─────────────────────────┐         ┌─────────────────────────┐
│  Column  (in Schema)    │         │  Cell  (in Row)         │
├─────────────────────────┤         ├─────────────────────────┤
│  Name: "src"            │         │  Type: TypeStr           │
│  Type: TypeStr          │   ≠     │  Str:  []byte("a")      │
│                         │         │  I64: (unused)          │
│  "this slot is a string"│         │  "this slot holds 'a'"  │
└─────────────────────────┘         └─────────────────────────┘
     DEFINITION                           VALUE
```

#### Encode: schema directs, row supplies cells
...
```text
         ...
         │                                   │
         ▼                                   ▼
   "use idx 1,2 for KEY"              "use idx 0 for VAL"
         │                                   │
         ▼                                   ▼
       ROW [ Cell₀ , Cell₁ , Cell₂ ]
              │        │        │
             skip      PK       PK
              │        │        │
              ▼        ▼        ▼
         EncodeVal   EncodeKey EncodeKey
              │        └────┬────┘
              │             │
              ▼             ▼
         KV value      KV key
         [123 bytes]   [link\0 + "a" + "b"]
```

#### Form analogy

```text
  SCHEMA                         ROW (filled form)
  ════════                       ═════════════════

  ┌─────────────────────┐        ┌─────────────────────┐
  │ Employee Form       │        │ Employee Form       │
  │ ─────────────────── │        │ ─────────────────── │
  │ 1. ID#    (number)  │  ───►  │ 1. ID#    → 123     │
  │ 2. Name   (text)    │  map   │ 2. Name   → "Alice" │
  │ 3. Dept   (text)    │        │ 3. Dept   → "Eng"   │
  └─────────────────────┘        └─────────────────────┘
   template on wall                 one person's paper
   (reused forever)                (many papers, same template)
```
...
```text
┌─────────────────────────────────────────────────────────────┐
│  RELATIONAL LAYER (0202)                                    │
│                                                             │
│   Schema ──maps──► Row []Cell ──Encode──► key + val []byte │
│   (1 per table)     (1 per insert)                          │
└─────────────────────────────┬───────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│  STORAGE LAYER (0103–0105)                                  │
│                                                             │
│   kv.Set(key, val)  →  log  →  disk                        │
│   (only sees bytes — no Schema, Row, or Cell)               │
└─────────────────────────────────────────────────────────────┘
```
...
```text
Schema  →  WHAT each index is     (Column: name + type)
Row     →  WHAT each index holds  (Cell: actual value)
```
