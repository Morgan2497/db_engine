...
func TestRowEncode(t *testing.T) {
	t.Log("=== 0304 Row Encode/Decode test start ===")
	...
	t.Logf("[SETUP] table=%q PKey=%v (src, dst)", schema.Table, schema.PKey)
	...
	t.Logf("[ENCODE expect VAL] %v (only non-PK column: time)", wantVal)
	...
	logRow(t, "after DecodeKey", decoded, "PK columns filled")
	...
	t.Log("=== 0304 Row Encode/Decode test end ===")
}
